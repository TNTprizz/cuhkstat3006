...
The program: \textit{(Lecturer used pseudo code to do the thing btw)}

\begin{verbatim}
    fn g(x: i32) {3 * x^2 - 5}
    fn ivt(Maxit: i32, a: &mut i32, b: &mut i32, ts: f32) {
        let mut c: i32 = (a + b) / 2;
        for i in 1..=Maxit {
            if g(c) == 0 || (b - a) / 2 < ts {break;}
            if g(c) * g(a) > 0 {a = c;} else {b = c;} 
        }
        c
    }
\end{verbatim}
